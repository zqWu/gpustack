...
class DockerCmdClient:
    def __init__(self, client: HTTPClient):
        self._client = client
        self._url = f"{client._base_url}/v1/docker-cmds"

    def list(self, params: Dict[str, Any] = None) -> DockerCmdsPublic:
        response = self._client.get_httpx_client().get(self._url, params=params)
        raise_if_response_error(response)

        return DockerCmdsPublic.model_validate(response.json())
    ...
    def get(self, id: int) -> DockerCmdPublic:
        response = self._client.get_httpx_client().get(f"{self._url}/{id}")
        raise_if_response_error(response)
        return DockerCmdPublic.model_validate(response.json())

    def create(self, model_create: DockerCmdCreate):
    ...
    def update(self, id: int, model_update: DockerCmdUpdate):
        response = self._client.get_httpx_client().put(
            f"{self._url}/{id}",
            # content=model_update.model_dump(mode='json'),
            # content=model_update.model_dump_json(),
            json=model_update.model_dump(mode='json'),
            headers={"Content-Type": "application/json"},
        )
        raise_if_response_error(response)
        return DockerCmdPublic.model_validate(response.json())
